...
const App = () => {
  ...
  const [times, setTimes] = useState(['Morning Before Food','Morning After Food','Afternoon Before Food','Afternoon After Food', 'Night Before Food', 'Night After Food']);
  ...
  return (
    <div>
    <Navbar/>
    <div className="p-5 min-h-screen flex md:flex-row flex-col gap-5 items-center justify-center bg-blue-100">
      <button
      ...
      <div className={`flex flex-row items-center space-y-4 ${!buttonsEnabled ? "opacity-50 cursor-not-allowed" : ""}`}>
        <form
          onSubmit={handleSubmit}
          className={`px-6 mx-2 py-4 bg-green-500  rounded-md items-center`}
        >
          {times.map((time, index) => (
            <div key={index} className="mb-4 gap-4 flex flex-row justify-between ">
